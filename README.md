...
- Полиморфизм — ключевой принцип, на котором держится всё решение:
    - Переопределение метода `Move()`: базовый класс `Ball` объявляет `virtual void Move()`, а наследники могут как использовать готовую логику (`RandomMoveBall`), так и полностью заменить её (`BilliardBall` переопределяет `Move`, убирая `Clear()`/`Show()`, потому что отрисовка идёт через `Paint`).
    - Переопределение метода `Go()`: `BilliardBall` добавляет к базовому движению отскоки от стен и вызов события `Hit`. `Bird` добавляет гравитацию `vy += G`, затухание при отскоках и проверку столкновения с `Pig`.
    - Расширение через `Draw(Graphics g)`: каждый класс сам отвечает за свою отрисовку. `BilliardBall` рисует залитый эллипс своим цветом, `Bird` — красный круг, `Pig` — розовый. Форма просто вызывает `ball.Draw(e.Graphics)` в цикле, не зная деталей.
